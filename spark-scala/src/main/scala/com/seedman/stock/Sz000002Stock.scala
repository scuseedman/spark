...
object Sz000002Stock {
  def main(args:Array[String]):Unit={
    val conf = new SparkConf().setAppName("sto").setMaster("local[4]")
    val sc = new SparkContext(conf)
     sc.setLogLevel("WARN")
      
      val rdd = sc.hadoopFile("sz001979_2018-03-12.xls", classOf[TextInputFormat], classOf[LongWritable], classOf[Text], 1).map(p => new String(p._2.getBytes, 0, p._2.getLength, "GBK"))
//      val rdd = sc.textFile("sz000002_2018-03-05", 1)
      val rdd1 = sc.textFile("sz000002_2018-03-06", 1)
